...
Make sure supershoe is a factor (KB)
```{r}
main_data$supershoe <- factor(main_data$supershoe, levels = c(0,1))
str(main_data)
```
Now supershoe is a factor. 
...
We can see that ozone was successfully removed from both train and test data. 
...
I am unable to get the code below to work, so I ran a couple basic lm()s above with the features listed out
