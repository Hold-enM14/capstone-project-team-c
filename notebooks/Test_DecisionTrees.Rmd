...
#Make all the genders uniform and standardized and merge nonbinary with female (KB)
...
#Compute average finishing time per mararthon/year (KB) (MH)

avg_times <- marathons_all %>% # (KB) switch to marathons_all dataset instead of runners dataset
...
train_index <- sample(1:nrow(final_data), size = 0.9 * nrow(final_data)) # use a 90/10 split
...
# wind speed bin
train_data$wind_bin <- ifelse(train_data$wind_speed >= 13,
                              "high_wind",
                              "low_wind")

# ozone bin
train_data$ozone_bin <- ifelse(train_data$ozone >= 38,
                               "high_ozone",
                               "low_ozone")

# temp bins (merging 'cool' and 'moderate', since moderate is a tiny bin)
train_data$temp_bin <- cut(train_data$avg_temp,
                           breaks = c(-Inf, 58, Inf),  # merge 'cool' and 'moderate'
                           labels = c("cool_moderate", "warm"),
                           right = FALSE)

# pm2.5 bin
train_data$pm25_bin <- ifelse(train_data$pm25 >= 54,
                              "high_pm25",
                              "low_pm25")

# temp × aqi interaction bin
train_data$temp_aqi_bin <- ifelse(train_data$temp_aqi_interaction >= 2789,
                                  "high_interaction",
                                  "low_interaction")
...
```{r}
# Comparing models with continuous and binned data (TRAIN ONLY) (MH)(KB)
...
```{r}
#Categorical bins from tree (MH)
#train_data$temp_bin <- cut(train_data$avg_temp,
#                           breaks = c(-Inf, 56, 66, Inf),
#                           labels = c("cool", "moderate", "warm")
#)

#train_data$ozone_bin <- ifelse(train_data$ozone >= 41, "higher", "lower")

#train_data$sea_level_pressure_bin <- ifelse(train_data$sea_level_pressure <30, "lower", "higher")

#train_data$pm25_bin <- ifelse(train_data$pm25 >= 62, "very high", "lower")
```

```{r}
#Comparing models with continuous and binned data (MH)
#model_cont <- lm(avg_chip_seconds ~ avg_temp + ozone + sea_level_pressure + pm25, data = train_data)

#model_bin <- lm(avg_chip_seconds ~ temp_bin + ozone_bin + sea_level_pressure_bin + pm25_bin, data = train_data)

#model_temp <- lm(avg_chip_seconds ~ temp_bin + ozone + sea_level_pressure + pm25, data = train_data)

#model_temp_ozone <- lm(avg_chip_seconds ~ temp_bin + ozone_bin + sea_level_pressure + pm25, data = train_data)

#model_airbin <- lm(avg_chip_seconds ~ avg_temp + ozone_bin + sea_level_pressure + pm25_bin, data = train_data)

#model_ozone <- lm(avg_chip_seconds ~ avg_temp + ozone_bin + sea_level_pressure + pm25, data = train_data)
```
...
```{r}
# caret cross-validation results (TRAINING DATA)
set.seed(123)
