...
Running XGBoost model with the best hyperparameters explicitly added, so you no longer need any tuning loop since it takes up alot of time (KB)
```{r}
set.seed(123)
...
We can see that the model is not overfitting as badly as before (Train RMSE = 37.48, Test RMSE = 142.94). This might be due to the Train set having more folds and more data. But, the gap is much smaller than before tuning (Train RMSE: ~76, Test RMSE: ~192). Overall, this is a big improvement in generalization.

We can also see that cross-validation error is reasonably close to test error, which is encouraging (CV RMSE = 151.61, Test RMSE = 142.94). This suggests that our 5-fold CV is reliable, the model generalizes fairly well, and there is no obvious leakage of information.

In addition, both the R² values show that the models fit extremely well (Train R² ≈ 0.9999 and Test R² = 0.9983). On the training data, it is so close to perfect mostly likely due to our engineered subgroup features explaining a large portion of the variability. So, the high R² most likely does not indicate overfitting on its own.

Overall, the tuned model clearly outperforms the original baseline model (Baseline Test RMSE ≈ 192, Tuned Test RMSE ≈ 142.94). That is around a 25–30% improvement in accuracy, which is substantial. This demonstrates that our hyperparameter tuning loop worked effectively.
