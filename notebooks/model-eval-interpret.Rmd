...
We can see the same trend on the test set (where GXBoost with feature engineering showed RMSE: 131.9877 and MAE: 101.7843). This XGBoost model achieves lower RMSE and MAE. This suggests that XGBoost is effectively capturing nonlinearities and interactions on its own, reducing the need for us to manually create them.
...
Originally we could see that the model was overfitting: There was a huge gap between Train_RMSE (15.07) and CV_RMSE (159.69) which means the model is overfitting the training data.
...
#Next:Forward Backward Stepwise Selection (not for XGboost but for LM?)
