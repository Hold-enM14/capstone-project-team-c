...
x_train <- model.matrix(avg_chip_seconds ~ ozone + pm25 + 
                        ozone_bin + pm25_bin, data = train_data)[,-1]  # remove intercept
y_train <- train_data$avg_chip_seconds

x_test <- model.matrix(avg_chip_seconds ~ ozone + pm25 + 
...
Removing ozone based on LASSO results (KB) *** UPDATE: getting rid of ozone and pm25. keeping bins based on decision tree
```{r}
...
# Introduce cross validation hyperparameter tuning by first running a tuning loop (KB)
...
Running XGBoost model with feauters with the best hyperparameters (KB)
```{r}
# Train final XGBoost model
xgb_model <- xgboost(
  data = dtrain,
  params = best_params,
  nrounds = best_nrounds,   # Optimal rounds from CV
  verbose = FALSE
)
...
# Display table
knitr::kable(model_eval, digits = 4, caption = "XGBoost Model Best Hyperparameters - Overfitting Check (Train MAE & R2 included)")

```

We can see that the model is not overfitting badly, only a reasonable amount (Train RMSE = 52, Test RMSE = 133). This might be due to the Train set having more folds and more data. But, the gap is much smaller than before tuning (Train RMSE: ~76, Test RMSE: ~192). Overall, this is a big improvement in generalization.

We can also see that cross validation error is almost the same as test error which is great news (CV RMSE = 152.7, Test RMSE = 133.2). This means that we have a healthy model, our 5-fold CV is reliable, the model generalizes well, and we are not leaking any information.

In addition, both the R2 show that the models fit extremely well (Train R² ≈ 0.9998 and Test R² = 0.9986). On the training data, it is so close to perfect mostly likley due to our subgroup categories explaining most of the variability. So, the high R2 most likey does not mean overfitting here.

Overall, the tuned model clearly outperforms the original baseline model (Baseline Test RMSE ≈ 192, Tuned Test RMSE ≈ 133). That is around a 30% improvement in accuracy, which is pretty large. This means that our tuning loop definitely worked. 
...
Probaly dont need this since I was just guessing the parameters.....
```{r}
...
#Double checking evaluation of test set
```{r}
#used best_xgb instead of xgb_base for predict
pred_xgb_eng <- predict(best_xgb, dtest)

rmse_xgb_eng <- rmse(test_data$avg_chip_seconds, pred_xgb_eng)
mae_xgb_eng  <- mae(test_data$avg_chip_seconds, pred_xgb_eng)

c(RMSE = rmse_xgb_eng, MAE = mae_xgb_eng)



#Calculating R2
rss <- sum((test_data$avg_chip_seconds - pred_xgb_eng)^2)
tss <- sum((test_data$avg_chip_seconds - mean(test_data$avg_chip_seconds))^2)
r2_xgb <- 1 - rss/tss
...
# Introduce cross validation hyperparameter tuning by first running a tuning loop on the raw features as done previously (KB)
...
for (i in 1:nrow(params_grid_raw)) {
  ...
  cv_raw <- xgb.cv(
    params = params,
    data = dtrain_raw,      # <<< RAW FEATURE MATRIX HERE
    nrounds = 1500,
    nfold = 5,
    early_stopping_rounds = 30,
    verbose = 0
...
Running XGBoost model with raw features with the best hyperparameters (KB)
```{r}
set.seed(42)
...
xgb_raw_model <- xgboost(
  ...
  nrounds = best_nrounds_raw,   # Optimal rounds from CV
  verbose = FALSE
)
...
# Train metrics
rmse_train <- rmse(train_data$avg_chip_seconds, pred_train_raw)
mae_train <- mae(train_data$avg_chip_seconds, pred_train_raw)
rss_train <- sum((train_data$avg_chip_seconds - pred_train_raw)^2)
tss_train <- sum((train_data$avg_chip_seconds - mean(train_data$avg_chip_seconds))^2)
r2_train  <- 1 - rss_train / tss_train


# CV RMSE from tuning
cv_rmse <- results_raw[[best_index_raw]]$best_rmse


# Test metrics
rmse_test <- rmse(test_data$avg_chip_seconds, pred_test_raw)
mae_test  <- mae(test_data$avg_chip_seconds, pred_test_raw)
rss_test  <- sum((test_data$avg_chip_seconds - pred_test_raw)^2)
tss_test  <- sum((test_data$avg_chip_seconds - mean(test_data$avg_chip_seconds))^2)
r2_test   <- 1 - rss_test / tss_test
...
# Display table
knitr::kable(model_eval, digits = 4, caption = "XGBoost Raw-Feature Model Best Hyperparameters - Overfitting Check (Train MAE & R2 included)")

```
We can see that the model is not overfitting severely, only to a reasonable extent (Train RMSE ≈ 44.9, Test RMSE ≈ 150.6). The gap between train and test performance is expected and much smaller than in the earlier model.This indicates that regularization and tuning have helped reduce the degree of overfitting.

We also can see that the cross validation error is very similar to the test error (CV RMSE ≈ 164.7, Test RMSE ≈ 150.6). This is good because it means that the model is stable across folds, the cross-validation process is reliable, and the model is generalizing consistently. This also means that there is no obvious data leakage.

In addition, both R2 scores show that the model fits the data extremely well (Train R² ≈ 0.9998, Test R² ≈ 0.9982). The almost perfect R2 is likely due to the subgroup structure, rather than overfitting.

Overall, the tuned model clearly outperforms the baseline model. The baseline had a Test RMSE of about 161.6, whereas the tuned model achieves 150.6, which is a meaningful improvement in accuracy. This represents roughly a 7% reduction in test error, showing that the tuning loop successfully improved generalization and stability.


Check for varaibel importance (ZD, KB)
```{r}
# Variable importance amongst final XGBoosted model
...
#Double checking evaluation of test set
```{r}
#used best_xgb_raw instead of xgb_base for predict
pred_xgb_raw_best <- predict(best_xgb_raw, dtest_raw)

rmse_xgb_raw <- rmse(test_data_raw$avg_chip_seconds, pred_xgb_raw_best)
mae_xgb_raw  <- mae(test_data_raw$avg_chip_seconds, pred_xgb_raw_best)
...
#Calculating R2
rss <- sum((test_data_raw$avg_chip_seconds - pred_xgb_raw_best)^2)
tss <- sum((test_data_raw$avg_chip_seconds - mean(test_data_raw$avg_chip_seconds))^2)
r2_xgb_raw <- 1 - rss/tss
